...
class NewNet(nn.Module):
    def __init__(self, spectral_num, channel=32):
        ...
        self.vector = nn.Parameter(torch.diag(torch.randn(self.spectral_num)))
        init_weights(self.backbone, self.conv1, self.conv3)  # state initialization, important!
        self.apply(init_weights)

    def forward(self, x, y):  # x lms y pan
        out_pan = torch.matmul(x.squeeze(0).permute(1,2,0), self.vector)
        pan_concat = y.repeat(1, self.spectral_num, 1, 1)  # Bsx8x64x64
        input = torch.sub(pan_concat, x)  # Bsx8x64x64
        rs = self.relu(self.conv1(input))  # Bsx32x64x64
        rs = self.backbone(rs)  # ResNet's backbone!
        w = self.relu(self.conv3(rs))  # Bsx8x64x64
        return out_pan[None,:].permute(0,3,1,2),w
...
class MyFusionNet(nn.Module):
    ...
    def forward(self, x, y, x_net):  # x_net
        ...
        w = self.relu(self.conv3(rs))  # Bsx8x64x64
        output = w*x_net
        out_pan = torch.matmul(output.squeeze(0).permute(1,2,0), self.vector)

        return output,out_pan[None,:].permute(0,3,1,2),w
...
class SimpleNet(nn.Module):
    def __init__(self, spectral_num, channel=32):
        ...
        self.conv3 = nn.Conv2d(in_channels=channel, out_channels=spectral_num, kernel_size=3, stride=1, padding=1,
                               bias=True)
        self.simd = nn.Sigmoid()
        self.relu = nn.ReLU(inplace=True)
        self.vector = nn.Parameter(torch.diag(torch.randn(8)))
        init_weights(self.conv1,self.conv2,self.conv3)  # state initialization, important!
        self.apply(init_weights)

    def forward(self, x_net):
        input = self.relu(self.conv1(x_net))
        w = self.relu(self.conv2(input))
        w = self.simd(self.conv3(w))
        output = w*x_net
        out_pan = torch.matmul(output.squeeze(0).permute(1,2,0), self.vector)
        return output,out_pan[None,:].permute(0,3,1,2),w
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes):
        ...
        self.vector = nn.Parameter(torch.diag(torch.randn(self.n_channels)))
        ...
        self.relu = nn.ReLU(inplace=True)
        init_weights(self.inc, self.down1, self.down2, self.down3, self.up1, self.up2, self.up3, self.outc)
        self.apply(init_weights)

    def forward(self, x, y):
        out_pan = torch.matmul(x.squeeze(0).permute(1, 2, 0), self.vector)
        pan_concat = y.repeat(1, self.n_channels, 1, 1)
