...
def main():
    ...
    parser.add_argument('--init_model', default='ADWM', type=str, help='Initial model name')
    parser.add_argument('--optw_lr', default=1e-3, type=float, help='Learning rate for wnet optimizer')
    parser.add_argument('--optX_lr', default=0.5, type=float, help='Learning rate for X optimizer')
    parser.add_argument('--work_dir', default='./outputs/simple_full', type=str, help='Working directory')
    ...
    logger.info(f"Starting DVF Full Resolution Training")
    ...
    data_dir = join(args.data_dir, f'test_{sensor.lower()}_OrigScale_multiExm1.h5')
    ...
    for dindex in range(20):
        ...
        for i in range(args.train_epochs):
            ...
            if (i + 1) % 500 == 0:
                D_lam = D_lambda(X, lrms_index.transpose(1, 2, 0), block_size=32, p=1)
                D_S = D_s(X, lrms_index.transpose(1, 2, 0), pan_index.transpose(1, 2, 0), 
                          satellite=sensor, scale=ratio, block_size=32, q=1)
                logger.info(f"Train Iter {i + 1}/{args.train_epochs} loss: {loss.item():.4e}")
            
        our_time = time.time() - start_time
        time_list.append(our_time)
        
        logger.info(f"Sample {dindex+1} completed - Time: {our_time:.3f}s")
        
        savemat(join(savemat_dir, f'output_mulExm_{dindex}.mat'), {'sr': X * max_value[sensor]})
